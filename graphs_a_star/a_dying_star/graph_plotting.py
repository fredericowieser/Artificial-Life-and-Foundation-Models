...
def create_graph_composite(json_data, final_size=(2280,1720), video_size=(400,400), margin=200):
    ...
    for node in G.nodes():
        ...
        clip, cw, ch = create_node_composite(node, G.nodes[node].get("label", ""), video_size, font_size=20)
        if clip is not None:
...
def main():
    json_file = "branches2.json"  # Update with your JSON file name/path.
    with open(json_file, "r", encoding="utf-8") as f:
        data = json.load(f)
    
    final = create_graph_composite(data, final_size=(2080,2720), video_size=(150,150), margin=200)
    if final is None:
