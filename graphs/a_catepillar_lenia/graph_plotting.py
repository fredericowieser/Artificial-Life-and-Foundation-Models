...
def build_graph_from_json(data, G=None):
    """Recursively builds a directed graph using 'video_path' as ID and storing the label."""
    if G is None:
        G = nx.DiGraph()
    # Clean up ID: remove extraneous whitespace and quotes
    base_id = str(data["video_path"]).replace('“','').replace('”','').strip()
    G.add_node(base_id, label=data.get("name", ""))
    for child in data.get("children", []):
        child_id = str(child["video_path"]).replace('“','').replace('”','').strip()
...
def create_graph_composite(json_data, final_size=(2280,1720), video_size=(400,400), margin=200):
    ...
    """
    G = build_graph_from_json(json_data)
    G.graph["graph"] = {"rankdir": "TB", "ranksep": "2", "nodesep": "1.5"}
